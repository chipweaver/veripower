...
- **rtl-design** — fan-out only, no dialogue: one Level-1 sub-Task per child, a gating semantic-review wave with a no-cap self-converge loop on its rtl-locus findings (exit: child-BLOCKED / convergence), then a finalize sub-Task.
