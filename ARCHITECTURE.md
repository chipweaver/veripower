...
- **rtl-design** — fan-out only, no dialogue: one Level-1 sub-Task per child, a per-child intent-review wave the stage acts on itself, then finalize.
...
`rtl-design` additionally runs a per-child intent-review wave; the promoted `semantic-review/<child>.md` set is its proposed oracle. Nothing reduces those reviews to a verdict — `finalize` refuses to write a pass while any child's review is absent, which is the whole of the mechanical part, since this stage has no in-stage user loop to notice a wave that never ran. What a review SAYS the stage acts on itself: an RTL defect is repaired by re-dispatching that child, and a defect in `design.md` or a `<child>.md` is named as `specification`'s in `fix_owner`. It has no deterministic spec↔RTL gate either: integration correctness is lint-cdc's elaboration, and the `.v`/`.vh` constraint the kernel's `rtl` selectors depend on is enforced declaratively by `rtl-files.schema.json` when `finalize` validates the sidecars.
