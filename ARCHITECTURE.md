...
`schedule.required_proofs` answers what this call is scheduling toward, from the log alone:

> **the proofs whose latest outcome is a `fail` — or, when none are failing, all eight.**

While a proof is failing it *is* the goal. Nothing downstream of it means anything until it re-verifies, so building the rest is work a second failure would only invalidate again; step 2 expands the narrowed set to its rebuild closure, so the producer a re-verify needs comes first. When the last failing proof re-verifies the set empties on its own and the goal widens back to the whole DAG.

Two consequences are worth naming, because an earlier design carried this as a caller-held mode and paid for both. There is **no transition to remember**: no switch into a repair episode, no switch back out, and no way to be left narrowed after the thing that narrowed it is gone. And the set is **plural**: two rules that fail together are both in it, so their re-verifies are both schedulable — a single-target answer left the second one out of the round and made it wait for the first, with no artifact edge between them.
...
**Who must act** is one pair, `(attribution, fix_owner)`, from one function (`schedule._attribution`). `attribution` is what was written, verbatim; `fix_owner` is its routable projection, present only when the naming is legal (§5.4) and — for a diagnosis — reliable. Both attribution channels reduce to that pair, and a later analysis outranks the stage's own self-report:

1. **A diagnosis is attached.** `_active_diagnoses` collects every non-superseded `diagnosis` whose `subject` matches this failure's `(proof, outcome_run)`; the latest speaks. **Reliability gate** (`_reliable`): reliable iff it names a `fix_owner` **and** either `source == human` or `confidence == high`. An oracle-side attribution — blaming the failed rule's own judge — arrives with no `fix_owner` and is refused by the first clause, so the gate does not test the attribution separately: both writers already enforce that, `cmd_diagnose` by rejecting a `fix_owner` outside the subject's input closure and `_derive_triage` by writing one only when the root cause is inside it, and no rule is in its own closure. An unreliable diagnosis leaves the complaint unowned, and `ESCALATE` cites every candidate.
...
**Triage's diagnosis at reap** (`kernel._derive_triage`). `simulation-triage` has no proof; it writes a `result.json` whose `stage_specific` carries `analysis_state`, `skipped_reason`, `root_cause`, `confidence`, `advisory`. At reap: `analysis_state != "complete"` → the outcome is `blocked` and no diagnosis is emitted (the complaint stays unowned; the next round re-dispatches triage). Otherwise the kernel appends a `diagnosis` (`source: triage`) whose `attribution` is the `root_cause` and whose `fix_owner` is that same `root_cause` when it names a rule inside `simulation`'s input closure. A self-pointing attribution (`root_cause == simulation`) is outside that closure by construction, so `fix_owner` is omitted and the complaint escalates. `confidence` lands as recorded; the reliability gate, not the reap, decides whether it auto-routes.
...
- **Output:** a `result.json` whose `stage_specific` carries the routing tier (`analysis_state`, `root_cause`, the gating `confidence`) and an advisory tier (`findings[]`, `waveform`, `experiment`). At reap the kernel derives the `diagnosis` from these: `findings[].anchor` becomes `fix_locus`, `experiment.artifacts[]` become `evidence`.
- **Authority — confidence-gated:** `confidence` is gating, not advisory. Only a `high`-confidence diagnosis auto-routes (via the reliability gate, §5.3); anything else escalates to the operator. The gate reads it only for a triage diagnosis: a human's is final on `source` alone, which is why `kernel.py diagnose` takes no confidence to state.
