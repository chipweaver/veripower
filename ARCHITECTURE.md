...
| `schedule.py` | The scheduler: `decide() → exactly one action`. Pure over (disk, log, args); composes `facts.signoff_gate`. Owns the goal set, the no-overtake gate, and the fresh-failure disposition, including the legality check on a failure's self-named `fix_owner`. |
