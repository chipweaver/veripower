...
- **simulation** — fan-out only, no dialogue: Wave 1 dispatches either the env-build child (first-run/rebuild) or the freeze-rebuild child (freeze path); a non-freeze run then runs the smoke gate, the LLM conformance review-gate (Step 4), and the verify child (Wave 2). On an incremental re-run where the plan is unchanged and the baseline TB is valid, simulation may **freeze** the prior run's TB — copying it into the fresh `runs/<N>/` (run-isolation §7.2 preserved: it still writes only to `runs/<N>/`) and recompiling against the changed RTL, rather than re-authoring it. The branch is selected deterministically by the `sim classify-delta` verb; the freeze materialization is the `sim freeze` verb. **On non-freeze runs,** shape closest to `specification`'s two-wave-around-a-gate; dispatch class identical to `rtl-design`'s.
...
| 2 (dual-chain parallel) | `{lint-cdc → synthesis → timing-analysis}` ‖ `{simulation}` | chain 1 is Task subagents; `simulation` is a main-thread sub-orchestrator dispatching its own intra-stage sub-Tasks (Wave 1: env-build child or freeze-rebuild child; non-freeze: smoke gate → conformance gate → verify child) | distinct in-flight ≤ 2 |
| 3 (merge) | power-analysis → frontend-signoff | all Task subagents | 1 |

**Fan-out sub-Tasks are intra-stage and state.py-invisible.** When `specification`, `simulation-plan`, `rtl-design`, or `simulation` dispatches Level-1 sub-Tasks (per-child work for the producers; the plan-adequacy reviewer for `simulation-plan`; the env-build or freeze-rebuild child and, on non-freeze runs, the verify child for `simulation`), those sub-Tasks run inside the main-thread skill's own execution window; they do not write `task.json`, do not append events, and do not appear in `state.py`'s in-flight bookkeeping. They therefore **do not count against** the `distinct in-flight ≤ 2` DAG-topology property — that property applies only to stage-level dispatches tracked by `state.py`. See §6.3 for the dispatch privilege carve-out.
...
**(TB-freeze conformance-gate skip.)** On a TB-freeze re-run where the TB is byte-identical and the plan unchanged, simulation's conformance gate is **skipped — its prior, already-promoted verdict carried forward** — because an unchanged check-vs-intent judgment cannot flip. See `skills/simulation/SKILL.md` Step 4.
