...
- **Output:** a `result.json` whose `stage_specific` carries `analysis_state` and `advisory.{findings[], waveform, experiment}`. At reap the kernel groups `findings[]` by `root_cause` and derives one `diagnosis` per group: that group's `anchor`s become `fix_locus`, `experiment.artifacts[]` become `evidence`.
