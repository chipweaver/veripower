...
- **Output:** a `result.json` whose `stage_specific` carries the routing tier (`analysis_state`, the gating `confidence`) and an advisory tier (`findings[]`, `waveform`, `experiment`). At reap the kernel groups `findings[]` by `root_cause` and derives one `diagnosis` per group: that group's `anchor`s become `fix_locus`, `experiment.artifacts[]` become `evidence`.
