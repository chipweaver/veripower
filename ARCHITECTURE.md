...
**(TB-freeze conformance-gate skip.)** On a TB-freeze re-run where the TB is byte-identical and the plan unchanged, simulation's conformance gate is **skipped — its prior, already-promoted verdict carried forward** — because an unchanged check-vs-intent judgment cannot flip. See `skills/simulation/SKILL.md` Step 4. Non-freeze runs go copy-first `patch` (see `docs/superpowers/specs/2026-07-02-sim-tb-incremental-patch-design.md`).
