...
| `diagnosis` | triage 自动（`reap`）；人工经 `diagnose` | 一条失败归因。必填（按 `diagnosis.schema.json`）：`id`、`subject {proof, outcome_run}`、`attribution`、`source ∈ {triage, human}`。可选：`fix_owner`、`supersedes`；`provenance`（背书者的裸身份）与 `reason`（推理本身，逐字带进 fix owner 的 `dispatch.json`）在 `source=human` 时均为必填，由 `diagnose` 强制。 |
...
- **`scope`** = 这条 rule 自上一次 run 以来发生漂移的模块相对输入（`facts.stale_inputs`）——正是这份漂移让证明失效并触发了这次重新派发。只有一个含义，不是两个:修复落在哪儿是分析该说的话，而它说在 `caused_by` 点名的那份记录里。漂移住在事件日志里，而 skill 不读日志。
...
- **输出：** 一份 `result.json`，其 `stage_specific` 携带 `analysis_state` 与 `advisory.{findings[]、waveform、experiment}`。收割时内核按不同的 `root_cause` 各追加一条 `diagnosis`——只有这次指名，别无其它。每条指向哪里仍留在 `findings[].anchor` 里，由被路由到的那一方从这份文件读；送达它的路径则从诊断的 `subject` 推出（§5.6）。
