...
整个系统围绕一个想法构建。一个确定性内核掌握设计流程的全部事实。哪些验证结论还成立，哪些被某次修改推翻了，下一步该跑什么，谁该负责修一个失败，全由它说了算。LLM agent 和人类工程师都站在内核外面。他们可以提交工作、提交判断，由内核经过 schema 校验后追加到事件日志。
...
依赖图从 `rules.py` 里每条规则声明的输入推导而来。一条输入指向某个阶段目录下的路径，那个阶段就是它的生产者 —— 各阶段根目录互不包含，所以生产者是精确的，也不需要任何一方再声明一遍自己的产出。依赖图与这些输入声明保持一致。

<p align="center">
  <img src="assets/pipeline-dag.png" alt="Pipeline dependency graph" width="660" />
</p>

*\*Specification 的输出（约束、PPA 目标、接口声明）也被 lint-cdc、synthesis、simulation 和 power-analysis 直接消费。Simulation-plan 的输出（验证计划、功耗场景标识）被 power-analysis 消费。这些边在图中省略了。*
...
编排器在两次查询之间不持有任何东西。上下文窗口被压缩了，或者进程崩溃了，下一次 `decide` 调用直接从磁盘重新推导出正确的动作。
...
失败的那个阶段指名谁需要行动。内核只做一件事：检查这个名字是否合法，也就是失败阶段自身或其传递输入产出阶段。没有固定的归因标签表。一个封闭集合只能表达事先想到的失败模式，而症状出现的位置不一定是原因所在的位置。

未明确或不相关的归属需要依据证据、按实际授权决定；阶段可以修复自身。升级时提供候选和相关证据。
...
**证明覆盖声明的输入。** 一条规则也能读取其他文件，但这些文件的变化不在已记录的依赖范围内。阶段交付的东西是有界的 —— 一条输入指向一棵树，树的版本是其下全部内容的 merkle，所以树内的文件不会因为名字出乎意料而漏掉；签核门则拒绝任何"canonical 目录里有自己 outcome 没记录的文件"的阶段。

**意图容器的版本化到边界为止。** 意图是一棵目录 `intent/`：`brainstorm.md`，加上工程师随它一起交付的东西。每条证明记录的是整棵树的一个 merkle，所以改文档、改其中任何一份权威材料、或者第一次把它交进来，八条证明全部失效，流程退回 `specification`。管不到的是文档从容器外面指出去的东西：容器之上的路径既不会被交给任何阶段，也不进版本；容器内的符号链接按目标路径记录，不按目标内容记录 —— 链接指向的那份共享 spec 换了内容，这里看不见。这类引用只能当成账本里的一行普通命题，由那一行点名的人去它真正所在的地方读它、确立它。
