...
依赖图从 `rules.py` 里每条规则声明的输入推导而来。一条输入指向某个阶段目录下的路径，那个阶段就是它的生产者 —— 各阶段根目录互不包含，所以生产者是精确的，也不需要任何一方再声明一遍自己的产出。没有别的东西维护这张图，所以图和规则实际读写的内容不可能产生分歧。
...
**声明的输入没有强制执行。** 一条规则声明了自己读什么，但没有任何机制真正阻止它去读别的文件。如果它读了，依赖图就会在危险的方向上出错：一条本该失效的证明没有失效。阶段交付的东西是有界的 —— 一条输入指向一棵树，树的版本是其下全部内容的 merkle，所以树内的文件不会因为名字出乎意料而漏掉；签核门则拒绝任何"canonical 目录里有自己 outcome 没记录的文件"的阶段。但两者都管不到一个跑到给定的树之外去读文件的工具。
...
**签核不是正确性。** 签核闭合的是一组声明出来的义务：规则注册表列出的八条证明，每条再对上需求账本里点它做判官的那些行。账本是对工程师文档的一次转录，由第二个读者拿原文核过 —— 不是从语言语义推导出来的。判官写成 `outside` 的行，流水线从不去确立它，签核也就无声地把它闭合进去了。所以签核的可信度，等于账本的完整度加上判官的诚实。
