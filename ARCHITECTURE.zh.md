...
四条 proposed 等级的规则都处理意图和功能正确性：specification、verification plan、RTL、simulation。这些判据表达工程意图，由工程师在签核前认可（§5）。
...
**签核闭合声明的验证义务。** 包括规则注册表列出的八条证明，每条再对上需求账本里点它做判官的那些行。账本是对工程师文档的一次转录，由第二个读者拿原文核过 —— 不是从语言语义推导出来的。判官写成 `outside` 的行仍由外部判官负责，不由流水线签核确立。
