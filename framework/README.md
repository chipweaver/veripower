...
| `scripts/schedule.py` | The scheduler behind the `decide` verb: objective-scoped, pure over (disk, log, args), returning exactly one action per call. Composes `route.py`. Import-only, reached solely through `kernel.py`. | `kernel.py`, by import | internal, tests |
| `scripts/route.py` | Pure deterministic rework-target selection, and the single home of the static failure-to-target maps. Holds no state; composed unchanged inside the scheduler and the CLI, never invoked directly. | `schedule.py`, `kernel.py`, by import | internal, tests |
