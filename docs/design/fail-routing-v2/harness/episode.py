...
def _cb_from_dispatch_json(module, rule, run):
    ...
    for rel in doc.get("caused_by", []):
        ...
        if len(parts) >= 3 and parts[-3] == "runs":
            root = tuple(parts[:-3])
            hit = next(
                (r for r in rules.RULES if rules.workdir_root(r) == root), None
            )
            if hit:
...
def run_episode(module, defects, limit=30):
    ...
    runs, actions = [], []
    occurrences = [(d.rule, 2) for d in defects]  # the initial failures, landed at run 2
    ...
    for step in range(limit):
        ...
        if a["action"] != "DISPATCH":
            return runs, actions, a["action"], occurrences, occ_step, deliveries, told_log
        rule = a["rule"]
        ...
        if not d["ok"]:
            ...
            return runs, actions, "DISPATCH_REJECTED", occurrences, occ_step, deliveries, told_log
        run = d["run"]
    ...
    with path.open("w") as fh:
        for i, ep in enumerate(eps):
            ...
            for (frule, frun) in occ:
                ...
                first_reverify = next(
                    (i for i, (r, n) in enumerate(runs) if r == frule and n > frun), None
                )
                if not steps:
                ...
                if own and any(
                    r == own and since < i < first_deliv for i, (r, _) in enumerate(runs)
                ):
