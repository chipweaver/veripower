...
def build(module, sc):
    ...
    if kind == "inflight":
        ...
        F._dispatch_ev(module, sc["running"], 2, F.recorded_inputs(module, sc["running"]))
...
def main():
    ...
    ap.add_argument("--filter", default=None, help="only scenarios whose id contains this")
    ...
    assert schedule.decide(str(template))["action"] == "DONE", "baseline is not all-valid"
    ...
    with out_path.open("w") as fh:
        for i, sc in enumerate(scs):
            ...
            rec_avail = [
                r for r in _rules.RULES
                if _facts.rule_available(str(mdir), evs, r)
            ]
            ...
            rec["raw"] = [
                {
                    k: a[k]
                    for k in ("action", "rule", "execution", "caused_by", "diagnosis_refs", "reason", "told")
                    if k in a
                }
                for a in seq
            ]
