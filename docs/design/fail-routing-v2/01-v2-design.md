...
第 5 项的推翻带来的删除项已并入 `04-simpler-model.md` §7:动作上的 `escalations[]`、`_dispatch_action` 的 `unowned` 分支、`_settle` 的顺序讲究、`design-flow/SKILL.md` 里那段上报要求,以及**状态网格 A4 这条性质本身** —— 它是度量阶段发明的判据,与最终策略相反,随之退休。
