...
**流水线遵循实际授权。** 需要你决定时呈现未决事项；明确委托范围内继续推进，签核背书要求保持有效。
...
**你的参与**：查看未决要求、数值界限和边界选择。需要你决定时会提供具体依据；已有委托范围内继续推进。交付时可查看 `design.md` 和评审记录。未解决的违约阻止本阶段通过，不能留给最后签核兜住。评审背书仍通过 `kernel.py pin` 完成（§1.6）。
...
**你的参与：**查看 `verification-plan.md` 和 `plan-review/findings.md`。阶段负责人依据证据处理发现，未解决的阻塞问题使本阶段不能通过。决定遵循实际授权，并将依据记入 `plan-review/decisions.md`。认可评审使用 `kernel.py pin`（下表 #6）；背书不替代缺陷处理。
...
| 文件 | 是什么 | 要你看吗 |
|---|---|---|
| `semantic-review/*.md` | 对照设计意图审 RTL 的评审 | **必看**，签核前需获授权背书 |
| `*.v` | RTL 本体 | 选看 |
...
**你的动作：读 `semantic-review/*.md`。** 它是签核前需获授权背书的四份判据之一（§1.6）。流水线不会在这里停下来等你，但提前读掉可以少一轮返工。
...
| 文件 | 是什么 | 要你看吗 |
|---|---|---|
| `tb/uvm/refmodel/**` | 判对错的参考模型 | **必看**，签核前需获授权背书 |
...
复核时先看 `analysis.md` 的测量条件、检查和结论，再按需查看原始报告。验收含义需要决策时，按实际授权由人工或代理处理。这一阶段过了流水线就到头了，签核是另一件由你开口要的事（§1.6）。
...
签核记录对当前验证证据的认可。任务包含签核时执行，例如“为 {module} 进行签核”。
流程核对阶段结果、判据内容、实测要求、工具身份和输入；工具报告也须有正确的范围与条件。

规格、计划、RTL 评审和仿真参考模型通过 `pin` 获得绑定内容的背书。`endorsed` 表示该内容
已获背书；`provenance` 说明决定者与授权，`reason` 说明依据。内容改变使背书失效，`reopen`
则显式撤回背书。

保留给你的决定会先呈现依据并等待；明确委托范围内由代理据证据处理并记录委托，拒绝仍然
有效。宿主执行权限独立生效。具备签核条件不会自动签核，最终接受决定由 `signoff` 记录。

**签核绑定已接受的证据。** 证据改变或背书被撤回会使签核失效。恢复同一份证据与背书可以恢复有效性；新的阶段结论需要按实际授权重新签核。
...
| grade（`proposed` / `tool` / `endorsed`） | 判据的成色。`proposed` = LLM 写的，签核前需获授权背书（背书后即 `endorsed`） |
...
| 3 | 交付回顾 | specification 独立评审后 | 查看设计与评审依据；未解决的违约先处理，背书在 #6 | 是 | §1.4 |
...
这些是决定与评审位置，不是强制弹出的权限询问。人工参与按实际授权安排；背书与签核仍是显式记录的决定。
