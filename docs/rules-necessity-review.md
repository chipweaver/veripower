...
| **simulation-plan** | `plan-review` (proposed) — testpoint coverage/adequacy vs spec | `design.md` / `manifest.json` / `*.md` (children) ✓ declared. `plan-data.json` = self cache. No RTL edge (windows narrow-edge ①: whitebox monitor hierarchy is derived at the sim stage, plan carries abstract names). **PASS.** |
...
| **lint-cdc** | `spyglass-ruleset` (tool) — CDC/lint clean | RTL fileset (`*.v`, `filelist.txt`), `README.md`, `sgdc_seed` (`constraints/*.sgdc`), `waiver.tcl` (in∩out) ✓ declared. Child-doc CDC intent reaches via rtl-design → RTL/README (both declared). SpyGlass ruleset = oracle identity. **⚠ OPEN dependency-completeness gap (F1):** the SGDC seed does not emit §1.6 clock-group Relationship, so multi-clock CDC is a verdict-dependency the seed under-covers — this is exactly the class relation 3 exists to catch. Tracked as a pre-existing constraint-gen bug (see memory `veripower-lint-sgdc-clockgroup-gap`); not introduced here. **CONDITIONAL PASS pending the seed fix.** |
...
- **Under-coverage (open, pre-existing):** `lint-cdc`'s SGDC seed does not cover the §1.6
  clock-group Relationship — a genuine verdict-dependency the seed misses (F1). Tracked
  separately; not introduced by the kernel work.
