...
- `Verification/simulation/conformance-review.json` — per-testpoint check-adequacy review, written by simulation's own Level-1 reviewer (schema `skills/simulation/references/conformance-review.schema.json`): prose per finding plus the reviewer's own `blocking` call, which is the whole of the gate. Unlike the other stages' reviews it is not an oracle and no human reads it before the stage routes on it, so a trip is dispositioned in-stage and `failure_phase=conformance` is what survives that.
