...
- `Design/rtl-design/semantic-review/<child>.md × N` — one intent review per child, written by its reviewer (contract `skills/rtl-design/references/rtl-review-task-contract.md`); rtl-design's proposed oracle, prose rather than a verdict.
...
- `Verification/simulation-plan/plan-review/review.md` + `decisions.md` — testpoint-adequacy review, written by simulation-plan's Step-3 self-dispatched Level-1 reviewer (prose: what it compared against, whether it blocks, where); `decisions.md` records the user's per-finding resolution at the Step-4 gate. Simulation-plan's proposed oracle — the kernel fingerprints it, no script reduces it to a verdict.
...
- `Design/specification/spec-review/<child>.md × N` + `decisions.md` — per-child spec intent review, written by that child's Step-6 reviewer (prose: what it compared against, whether it blocks, where); `decisions.md` records the user's per-finding resolution at the Step-7 gate. Specification's proposed oracle — the kernel fingerprints it, no script reduces it to a verdict.
