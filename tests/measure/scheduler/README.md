# `tests/measure/scheduler/` — 合成场景度量

调度器的两台合成装置。**场景集不是手写的**:`space.py` 从 `rules.py` 推导出全部 1781 个状态,
所以加一条规则或改一条 `inputs` 选择器,场景集自己就变了。

不是 pytest 断言集,是**度量装置**——输出要人读:网格问「每个状态引出什么动作」,episode 问
「跑到收敛要花多少机时,相对注册表下界浪费了多少」。
