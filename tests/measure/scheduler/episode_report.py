...
EXITS = {
    ...
    "E2": (
        "结论不丢:每条失败的信封都送达过某个 owner 的 dispatch.json",
        lambda r: not r["undelivered"],
    ),
    "E3": (
        "形态正确:同 owner 合并为一轮,不同 owner 各自一轮",
        lambda r: r["merge_misses"] == 0,
    ),
    "E4": ("无额外重跑:每个缺陷只失败一次", lambda r: r["extra_failures"] == 0),
    "E6": (
        "最近注入:失败与它被交付之间,owner 没有白跑过一轮",
...
def summarize(recs, label):
    ...
    print(
        f"=== {label}: {len(recs)} episodes, endings "
        f"{dict(collections.Counter(r['ending'] for r in recs))} ==="
    )
    print(
        f"  single-defect ({len(one)}): waste "
...
def main():
    ...
    if args.diff:
        ...
        print(
            f"  total waste {sum(r['waste_min'] for r in recs)} -> "
