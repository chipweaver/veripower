...
@prop(
    "A0",
    "a legal conclusion is DELIVERED: the failing run's own envelope reaches its "
...
def a0(rec):
    ...
    for side in ("a", "b"):
        ...
        if r and c and space.owner_of(r, c) in startable:
            want.add(
                f"{'/'.join(__import__('rules').workdir_root(r))}/runs/2/result.json"
            )
    if not want:
        return None
...
def main():
    ...
    for r in by_kind["forward-pair"]:
        ...
        rows[r["rel"]][
            ("co-open" if both else "serial", "overlap" if overlap else "-")
        ] += 1
    for rel, c in rows.items():
        print(
            f"  {rel:<12} "
            + "  ".join(f"{k[0]}/{k[1]}={n}" for k, n in c.most_common())
        )
    ...
    for r in sorted(
        by_kind["single-fail"], key=lambda r: (space.PROOFS.index(r["a"]), r["cat_a"])
    ):
    ...
    for rel in sorted(shape):
        ...
        print(
            f"  {rel:<28} n={tot:<5} "
            + "  ".join(f"{k}={v}" for k, v in shape[rel].most_common())
        )
