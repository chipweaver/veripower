...
def mirror(
    ...
    *,
    ...
    return MAIN, workdir, module


def run(main, module, workdir, *extra):
...
def test_bootstrap_deploys_infra_and_dirs(tmp_path):
    main, wd, module = mirror(tmp_path)
    r = run(main, module, wd)
...
def test_bootstrap_renders_scaffold_when_given(tmp_path):
    ...
    r = run(main, module, wd, "--plan", str(plan_dir))
    assert r.returncode == 0, r.stderr
...
def test_bootstrap_unusable_top_exit_1(tmp_path):
    ...
    main, wd, module = mirror(tmp_path, scaffold_top="dut-1 top")
    r = run(main, module, wd)
...
def test_bootstrap_missing_scaffold_file_exit_1(tmp_path):
    main, wd, module = mirror(tmp_path)
    r = run(main, module, wd, "--plan", str(tmp_path / "nope"))
...
def test_pycache_beside_the_templates_is_not_deployed(tmp_path):
    ...
    main, wd, module = mirror(tmp_path)
    assert run(main, module, wd).returncode == 0
