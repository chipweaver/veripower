...
def write_specification(tmp_path, hints=("CHK-0",)):
...
def finalize_workdir(tmp_path, *, scaffold=None, review=True):
    wd = tmp_path / "plan"
    wd.mkdir()
    split(wd, scaffold or GOOD)
...
def test_invalid_plan_blocks_closure(tmp_path):
    ...
    spec = write_specification(
        tmp_path, hints=("CHK-0", "CHK-1")
    )  # CHK-1 covered by nothing
...
def test_finalize_blocked_on_empty_fail_reason(tmp_path):
    rc = vs.finalize(tmp_path, write_specification(tmp_path), fail_reason="  ")
    assert rc == 2
...
def test_a_stated_reason_is_the_failure(tmp_path):
    ...
    wd = finalize_workdir(tmp_path)
    spec = write_specification(tmp_path)
    assert vs.finalize(wd, spec, fail_reason="reviewer BLOCKED: x") == 0
    env = json.loads((wd / "result.json").read_text())
    assert env["status"] == "fail"
