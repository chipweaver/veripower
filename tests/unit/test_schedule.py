...
def test_wake_reap(tmp_path, monkeypatch):
    ...
    dispatch(
        ...
        "specification",
        1,
        {"intent": facts.fingerprint(Path("m") / "intent")},
    )
    # workdir result.json present -> REAP even without wake (收口 branch)
    mk("m", workdir("specification", 1) + "/result.json", "{}")
    a = schedule.decide("m")
...
def test_wake_reaps_a_run_whose_executor_wrote_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    write_file("m", "intent/brainstorm.md", "b1")
    dispatch("m", "specification", 1, {"intent": fp("m", "intent")})
...
def test_a_landed_result_is_reaped_not_yielded_over(tmp_path, monkeypatch):
    ...
    dispatch("m", "lint-cdc", 1, recorded_inputs("m", "lint-cdc"))
    dispatch("m", "simulation-plan", 1, recorded_inputs("m", "simulation-plan"))
    rj = Path("m") / workdir("simulation-plan", 1) / "result.json"
    mk("m", workdir("simulation-plan", 1) + "/result.json", "{}")
...
def test_in_flight_no_result_yields(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    write_file("m", "intent/brainstorm.md", "b1")
    dispatch("m", "specification", 1, {"intent": "sha256:x"})
    a = schedule.decide("m")
...
def test_fresh_failure_with_routable_triage_dispatches_fix_owner(tmp_path, monkeypatch):
    ...
    valid_chain_through_simulation("m")
    sim_fail("m", run=1)
    store.append_event(
        "m",
        {
...
def diagnosis(module, did, run, owner):
    store.append_event(
        module,
        {
            "type": "diagnosis",
...
def test_one_failure_with_two_root_causes_reaches_both_owners(tmp_path, monkeypatch):
    ...
    valid_chain_through_simulation("m")
    sim_fail("m", run=1)
    diagnosis("m", "d-rtl", 1, "rtl-design")
    diagnosis("m", "d-plan", 1, "simulation-plan")
    ...
    still = schedule.owed(ev, schedule.failures("m", ev))
    assert still == []
    # that round died, so it re-opens — for its owner alone, not for the other one
    outcome("m", "rtl-design", runs["rtl-design"], "blocked", {}, [])
    ev = store.read_events("m")
    assert [o["owner"] for o in schedule.owed(ev, schedule.failures("m", ev))] == [
        "rtl-design"
    ]
...
def test_local_repair_waits_for_its_upstream_repair(tmp_path, monkeypatch):
    ...
    valid_chain_through_simulation("m")
    sim_fail("m", run=1)
    diagnosis("m", "d-local", 1, "simulation")
    diagnosis("m", "d-rtl", 1, "rtl-design")
...
def test_fresh_failure_self_pointing_escalates(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    valid_chain_through_simulation("m")  # helper: spec/plan/rtl proofs valid on disk
    sim_fail("m", run=1)  # helper: fresh simulation fail outcome
    store.append_event(
        "m",
        {
            "type": "diagnosis",
...
def test_repair_after_fix_lands_redispatches_failed_rule_not_fix_owner(
    ...
    monkeypatch.chdir(tmp_path)
    valid_chain_through_simulation("m")
    fail("m", "simulation", 1, owner="rtl-design")  # records the OLD matvec.v version
    valid(
...
def test_blocked_goes_forward_no_escalate(tmp_path, monkeypatch):
    ...
    write_file("m", "intent/brainstorm.md", "b1")
    dispatch("m", "specification", 1, {"intent": fp("m", "intent")})
    outcome("m", "specification", 1, "blocked", {}, [], reason="crash")
    a = schedule.decide("m")
    assert a["action"] == "DISPATCH" and a["rule"] == "specification"
...
def test_fresh_selfdescribing_failure_dispatches_the_owner_its_envelope_named(
    ...
    valid("m", "specification", 1)
    valid("m", "rtl-design", 1)
    fail("m", "lint-cdc", 1, owner=None)  # the envelope names nobody
...
def test_fresh_rtldesign_spec_locus_dispatches_specification(tmp_path, monkeypatch):
    ...
    mk("m", "intent/brainstorm.md", "b1")
    valid("m", "specification", 1)
    fail("m", "rtl-design", 1, owner=None)  # the envelope names nobody
    # nobody named and no diagnostic declared for this rule -> ESCALATE
    pre = schedule.decide("m")
    assert pre["action"] == "ESCALATE"
...
def timing_fail_over_stale_synthesis(module):
    ...
    valid(module, "synthesis", 1)
    mk(module, "Design/synthesis/reports/qor.rpt", "changed measurement")
    fail(module, "timing-analysis", 1, owner="synthesis")
...
def test_advisory_holds_while_its_predecessor_is_still_running(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    timing_fail_over_stale_synthesis("m")
    dispatch("m", "lint-cdc", 1, recorded_inputs("m", "lint-cdc"))
    assert "synthesis" not in turn("m")


def test_advisory_releases_once_its_predecessor_has_spoken(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    timing_fail_over_stale_synthesis("m")
    valid("m", "lint-cdc", 1)  # it ran and passed
    d = schedule.decide("m")
...
def test_advisory_predecessor_is_scheduled_rather_than_waited_on(tmp_path, monkeypatch):
    ...
    timing_fail_over_stale_synthesis("m")
    valid("m", "lint-cdc", 1)
    mk("m", "Design/lint-cdc/lint-report.txt", "drift")  # lint-cdc proof now invalid
    ev = store.read_events("m")
...
def test_advisory_orders_two_stages_that_failed_together(tmp_path, monkeypatch):
    ...
    valid("m", "simulation-plan", 1)
    valid("m", "rtl-design", 1)
    fail("m", "lint-cdc", 1, owner="rtl-design")
    fail("m", "synthesis", 1, owner="rtl-design")
    valid(
        "m", "rtl-design", 2, tag="fix"
...
def test_co_failing_rules_reverify_in_parallel(tmp_path, monkeypatch):
    ...
    valid("m", "rtl-design", 2, tag="fix")  # the fix lands; both fails go stale
    first = schedule.decide("m")
    assert first["action"] == "DISPATCH"
    dispatch("m", first["rule"], 2, recorded_inputs("m", first["rule"]))
    ...
    for unused in range(12):
        ...
        valid(
            ...
            sum(
                ...
                if event["type"] == "dispatch" and event["rule"] == a["rule"]
            )
            + 1,
        )
    assert seen[:2] == [
...
def test_closing_changes_what_done_means_not_which_proofs(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    build_all_valid("m", 1)
    mk("m", "Design/specification/unrecorded.md", "not delivered")
...
def test_advisory_edge_never_enters_freshness(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    valid_chain_through_power("m")  # helper: power's ARTIFACT closure all valid
    invalidate_proof("m", "timing-analysis")  # helper: drift a timing-only input
    fail("m", "power-analysis", 1, owner="simulation")  # its own inputs untouched
    ev = store.read_events("m")
    owed = schedule.owed(ev, schedule.failures("m", ev))
...
def test_two_hop_upstream_invalidity_does_not_discard_the_failure(
    ...
    fail("m", "timing-analysis", 1, owner=None)
    fails = schedule.failures("m", store.read_events("m"))
...
def test_repair_rebuild_chain_dispatches_producer_first(tmp_path, monkeypatch):
    ...
    fail("m", "timing-analysis", 1, owner="synthesis")
    a = schedule.decide("m")
...
def test_new_outcome_deactivates_old_diagnosis(tmp_path, monkeypatch):
    ...
    sim_fail("m", 2)  # simulation re-runs, NEW fail outcome
    a = schedule.decide("m")
...
def test_triage_blocked_redispatches_no_livelock(tmp_path, monkeypatch):
    ...
    outcome("m", "simulation-triage", 1, "blocked", {}, [], reason="missing")
    a = schedule.decide("m")
    assert a["action"] == "DISPATCH" and a["rule"] == "simulation-triage"


# --- Elided prefix-builder helpers (mechanical, test_facts_freshness pattern) ---
# Each valid(rule) dispatches+passes a rule with its declared (non-self) inputs on
...
def valid(module, rule, run, *, extra_inputs=(), tag=None):
    ...
    if rule in reviews:
        rels.append("/".join((*rules.RULES[rule].workdir_root, reviews[rule])))
    for rel in rels:
        mk(module, rel, f"{rule}:{rel}:{marker}")
    inputs = recorded_inputs(module, rule, extra_inputs)
    outputs = {rel: fp(module, rel) for rel in rels}
...
def sim_fail(module, run):
    ...
    fail(module, "simulation", run, owner=None)


def power_fail(module, run):
    fail(module, "power-analysis", run)


def valid_chain_through_simulation(module):
...
def valid_chain_through_power(module):
    ...
    valid(module, "rtl-design", 1)
    valid(module, "synthesis", 1)
    valid(module, "simulation", 1)


def invalidate_proof(module, rule):
    ...
    priv = "/".join(rules.RULES[rule].workdir_root) + "/_private_in.txt"
    mk(module, priv, "priv-v1")
    valid(module, rule, 1, extra_inputs=(priv,))
    mk(module, priv, "priv-v2-drift")


def build_all_valid(module, run, *, include=None):
...
def test_decide_repair_survives_triage_fail_outcome(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    build_all_valid("m", 1)
    outcome("m", "simulation-triage", 1, "fail", {}, [])
...
def test_unregistered_rule_in_flight_is_not_reapable_forever(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    build_all_valid("m", 1)
    dispatch("m", "not-a-rule", 1, {})  # never reaped
...
def test_fresh_fail_fix_owner_in_flight_yields(tmp_path, monkeypatch):
    ...
    dispatch("m", "rtl-design", 2, {})  # fix_owner already in flight
    assert schedule.decide("m")["action"] == "YIELD"
...
def test_sim_fail_triage_in_flight_is_not_dispatched_twice(tmp_path, monkeypatch):
    ...
    valid_chain_through_simulation("m")
    sim_fail("m", run=1)
    dispatch("m", "simulation-triage", 1, {"sim_run": 1})
    assert "simulation-triage" not in turn("m")
...
def test_option_c_defers_producer_with_inflight_consumer(tmp_path, monkeypatch):
    ...
    valid("m", "specification", 1)
    valid("m", "rtl-design", 1)
    mk("m", "Design/rtl-design/semantic-review/child.md", "drift")  # rtl proof invalid
    dispatch("m", "lint-cdc", 1, {})  # in-flight consumer of Design/rtl-design/*.v
    d = schedule.decide("m")
...
def test_signoff_gate_blocks_on_a_file_the_stage_never_delivered(tmp_path, monkeypatch):
    ...
    mk("m", "Design/rtl-design/sneaky.vh", "`define SNEAKY 1")
...
def test_basis_names_the_input_set_each_verdict_was_about(tmp_path, monkeypatch):
    ...
    unused, outcome = facts.proof_outcome(events, "specification")
