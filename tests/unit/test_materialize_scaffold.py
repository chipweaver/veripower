"""Tests for materialize_scaffold.py — fills scaffold agents[] signals + transaction.fields
...
SCRIPT = ROOT / "skills/simulation-plan/scripts/materialize_scaffold.py"
...
def _run(pd, sc, check=True):
    return subprocess.run(
        ["python3", str(SCRIPT), "--plan-data", str(pd), "--scaffold", str(sc)],
        capture_output=True,
        text=True,
        check=check,
    )
