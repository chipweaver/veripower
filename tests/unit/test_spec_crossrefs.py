...
def port(name, direction, role, domain="clk", width=1, group="cfg"):
    return {
        "name": name,
        "direction": direction,
        "width": width,
...
PORTS = [port("clk", "input", "clock"), port("din", "input", "data", width=8)]


def workdir(tmp_path, clocks=None, rows=None, ports=None, hints=None):
    ...
    (tmp_path / "top-io.json").write_text(json.dumps(PORTS if ports is None else ports))
    (tmp_path / "check-hints.json").write_text(
        json.dumps(HINTS if hints is None else hints)
    )
    return tmp_path
...
def verdict(tmp_path, **kw):
    ...
    return crossrefs.verdict(workdir(tmp_path, **kw))


def said(v, where, what_fragment):
    return any(
...
def test_port_clock_domain_not_in_clocks_json(tmp_path):
    ...
    ports = PORTS + [port("q", "output", "data", domain="ghost")]
    v = verdict(tmp_path, ports=ports)
    assert said(v, "top-io.json q", "'ghost' is not in clocks.json")
...
def test_hint_naming_a_row_the_ledger_lacks_is_reported(tmp_path):
    hints = [{**HINTS[0], "requirements": ["R-99"]}]
    v = verdict(tmp_path, hints=hints)
    assert said(
...
def test_hint_naming_a_row_another_stage_judges_is_reported(tmp_path):
    ...
    # did not ask for.
    hints = [{**HINTS[0], "requirements": ["R-01"]}]
    v = verdict(tmp_path, hints=hints)
    assert said(v, "check-hints.json CHK-0", "'R-01', which is not a simulation row")
...
def test_hint_naming_a_coverage_row_is_reported(tmp_path):
    ...
    hints = [{**HINTS[0], "requirements": ["R-02"]}]
    v = verdict(tmp_path, hints=hints)
    assert said(v, "check-hints.json CHK-0", "'R-02', which is not a simulation row")
...
def test_simulation_row_no_hint_names_is_reported(tmp_path):
    ...
    v = verdict(tmp_path, rows=rows)
    assert said(v, "requirements.json R-03", "no check-hints entry names it")


def test_a_row_two_hints_name_is_covered_once(tmp_path):
    hints = HINTS + [{**HINTS[0], "check_id": "CHK-1"}]
    v = verdict(tmp_path, hints=hints)
...
def test_duplicate_check_id_in_the_file_is_reported(tmp_path):
    ...
    v = verdict(tmp_path, hints=HINTS + HINTS)
    assert said(v, "check-hints.json CHK-0", "already used in this file")
...
def test_every_disagreement_is_reported_not_just_the_first(tmp_path):
    ...
    ports = PORTS + [port("q", "output", "data", domain="ghost")]
    hints = [{**HINTS[0], "requirements": ["R-99"]}]
    v = verdict(tmp_path, rows=rows, ports=ports, hints=hints)
