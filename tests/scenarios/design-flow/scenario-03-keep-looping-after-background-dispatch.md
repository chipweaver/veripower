...
model: opus
provenance: "RED 5x = 5B, GREEN 8x = 8A — 2026-08-16, first stamp taken under asserted isolation (`stream_text.py` refuses a tag if the init event lists tools or a tool call lands). Everything before this line is uncomparable: the runner isolated with `--allowedTools \"\"`, which stopped disabling tools somewhere before CLI 2.1.233, so the subject silently gained tools and the recorded 6A/2REVIEW_NEEDED/0B could no longer be reproduced even from the byte-identical SKILL.md it was stamped on (measured: 8x on that exact file gave 2A/5B/1RN). The 2026-08-07 REFACTOR note claiming the mechanism needs the loop comment PLUS a Discipline sentence was read the same way; with the Discipline paragraph gone entirely, GREEN is 8/8 A, so the loop pseudocode carries this invariant alone. RED still fails 5/5 with the same rationalization as 2026-08-04 ('lint may change the RTL, hold simulation back') — the scenario keeps its teeth."
---
