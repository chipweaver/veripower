...
misses — and that the 2026-06-10 audit found broken (B1: `state.py.RESULT_DIR`,
a symbol that lives in `topology.py`; B2: `<child>.md §1.7+`, a section that
does not exist — a child sub-design is §1–§5):

  A. **module.symbol** — a backtick ref like `topology.RESULT_DIR` /
     `state.result_path` must name a symbol defined at module level in
...
# ── module.symbol ────────────────────────────────────────────────────────
MODULES = sorted(p.stem for p in (PLUGIN_ROOT / "framework" / "scripts").glob("*.py"))
MOD_RE = re.compile(r"`(" + "|".join(MODULES) + r")(?:\.py)?\.([A-Za-z_][A-Za-z0-9_]*)")
MODULE_NAMES: dict[str, set[str]] = {}


def module_attrs(module: str) -> set[str]:
    ...
    `result_path` via `from topology import …`). Static AST parse, no execution."""
...
def symbol_defined(module: str, symbol: str) -> bool:
    return symbol in module_attrs(module)
...
SKIP_DOCS = {"verification-plan.md", "result.json", "task.json", "events.jsonl"}

# doc (optionally backticked, optionally a path) immediately followed by §section
SEC_RE = re.compile(r"([\w./<>-]+\.md)`?\s*§\s*([0-9][0-9.]*\+?)")


def heading_lines(path) -> list[str]:
    return [ln for ln in path.read_text().splitlines() if re.match(r"^#{1,6}\s", ln)]


def section_present(headings: list[str], sec: str) -> bool:
...
def resolve_doc_source(doc: str):
    if doc in SKIP_DOCS:
        return None
    if doc in DOC_SOURCES:
        return PLUGIN_ROOT / DOC_SOURCES[doc]
...
def test_module_symbol_lint_still_works() -> None:
    ...
    assert MOD_RE.findall("`rules.FORWARD_PRIORITY`") == [("rules", "FORWARD_PRIORITY")]
    assert symbol_defined("rules", "FORWARD_PRIORITY")
    assert not symbol_defined("rules", "NO_SUCH_SYMBOL")
...
def test_skill_module_symbol_refs_resolve() -> None:
    ...
    for skill in SKILL_DIRS:
        ...
        if not skill_md.is_file():
            continue
        for mod, sym in MOD_RE.findall(skill_md.read_text()):
            if (
                sym == "py"
            ):  # a bare `mod.py` file ref (extension, not a symbol) — path test owns it
                continue
            if not symbol_defined(mod, sym):
                bad.append(
...
def test_section_lint_still_works() -> None:
    ...
    assert SEC_RE.findall("`design.md` §1.2 holds the diagram") == [
        ("design.md", "1.2")
    ]
    headings = heading_lines(resolve_doc_source("design.md"))
    assert section_present(headings, "1.2") and not section_present(headings, "9.9")
...
def test_skill_section_refs_resolve() -> None:
    ...
    for skill in SKILL_DIRS:
        ...
        if not skill_md.is_file():
            continue
        for doc, sec in SEC_RE.findall(skill_md.read_text()):
            src = resolve_doc_source(doc)
            if src is None:
                continue  # not deterministically resolvable — skipped by design
            if not section_present(heading_lines(src), sec):
                bad.append(
