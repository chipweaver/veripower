...
合成度量的盲区是它自己造的:状态网格取**一个状态的一个动作**且从不 reap,收敛回放集数的是**轮数**而不是重叠。于是"什么时候开跑"这一类改动在两者上都是零变化,而它正是墙钟成本所在。2026-08-05 那批改动(目标集不再收窄、一条失败可以有多个 owner、`ESCALATE` 带 `remedy`、`YIELD` 带 `executor_wrote`)在网格上 0 格变化、在回放集上逐条 `same`,在这里是 **49/60 → 56/60**。
