"""Parse power and activity values from PrimeTime PX text reports.
...
def run(plan_path, workdir, targets_json, out_path) -> int:
    ...
    out_path.write_text(json.dumps(payload, indent=2) + "\n")
    print(f"[power_rpt_parser] Written: {out_path} (verdict={verdict})")
    return 0
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if exc.code not in (0, None):
            ...
            return 2
        return 0
    return run(args.plan, args.workdir, args.targets, args.out)
