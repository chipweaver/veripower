...
- **Freeze-reuse when nothing changed.** When the incremental-update branch's `specification` diff comes back empty and no `{directive_path}` is injected, do not regenerate anything — reuse the prior run's `verification-plan.md` / `scaffold-specification.json` **and the prior `plan-review.json`** verbatim (copy them forward; never ask the LLM to reproduce them byte-for-byte, which a probabilistic model cannot guarantee). **Reuse the review record too, do not re-run the Step-4 adequacy gate:** re-judging identical content regenerates `plan-review.json`, whose fingerprint is the pin's content selector (§5.4), so a fresh review would silently drop any `pin` on it (§4.3 "重验不重生成"). This is the generalization of `simulation`'s freeze branch to your two authored artifacts AND its LLM judgment record — a forced full rebuild re-verifies existing conclusions, it does not regenerate them.
