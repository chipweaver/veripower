...
- **Freeze-reuse when nothing changed.** When the incremental-update branch's `specification` diff comes back empty and no `{directive_path}` is injected, do not regenerate anything — reuse the prior run's `verification-plan.md` / `scaffold-specification.json` verbatim (copy them forward; never ask the LLM to reproduce them byte-for-byte, which a probabilistic model cannot guarantee). This is the generalization of `simulation`'s freeze branch to your two authored artifacts — a forced full rebuild re-verifies existing conclusions, it does not regenerate them.
