...
3. Else, if `{workdir}/changed-inputs.md` is present, it lists the input files that changed since this stage's last run — amend only the plan sections its `design.md` / `<child>.md` paths map to. If it is absent or empty but `{workdir}/verification-plan.md` is present AND carries frontmatter `Status: approved` (a prior approved round, carried forward — a re-verify, not a first delivery; a plan lacking this frontmatter never cleared the Step-5 user loop, so it cannot qualify), amend no section — re-run Step 4's gate on the carried plan and finalize; every section outside scope stays byte-identical.
4. Else (a first delivery — no approved `verification-plan.md` carried in, or one interrupted before the Step-4 gate / Step-5 approval) full generation of plan + scaffold.
