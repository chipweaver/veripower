#!/usr/bin/env python3
"""Validate scaffold-specification.json at the sim-plan Completion Gate.

Runs AFTER materialize_scaffold.py (so agents[] carry materialize-injected
...
Three layers (each runs only if the prior passed):
  ...
  3. Coverage — bidirectional matrix over the LLM judgment vs plan-data.json
     ...
     covers[] entry resolves to a real check_id. Applied by main() after layers 1-2.

Exits non-zero with a readable, fix-oriented message. Pairs with derive_scaffold.py's
thin consumer-side backstops (defense-in-depth for scaffolds that bypass this gate).
"""
...
def coverage_errors(scaffold: dict, plan_data: dict) -> list:
    ...
    construction (materialize_scaffold), so it is not re-checked here. The reverse
    (dangling-covers) check duplicates materialize's guard — it earns its keep only when
    validate runs standalone on a non-materialized scaffold; in the pipeline, materialize
...
def main() -> int:
    ...
    p.add_argument(
        "--scaffold", required=True, help="Path to scaffold-specification.json"
    )
    p.add_argument(
        "--plan-data", required=True, help="Path to plan-data.json (coverage universe)"
    )
    p.add_argument(
        "--schema",
        default=str(_DEFAULT_SCHEMA),
        help="Path to scaffold-specification.schema.json (default: sibling references/)",
    )
    args = p.parse_args()
    ...
    if errors:
        sys.exit(
            "validate_scaffold: scaffold-specification.json invalid:\n  - "
            + "\n  - ".join(errors)
            + "\nFix scaffold-specification.json (re-author per SKILL.md scaffold-spec contract) and re-run."
        )
    print(
        f"validate_scaffold: OK ({len(scaffold.get('agents', []))} agents, "
