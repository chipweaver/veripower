"""simplan.plan — the plan's three sidecars: their names, their schemas, and the merge.
...
load_plan validates and merges the sidecars for cross-file checks.
"""
...
def read_validated(path: Path, schema_name: str):
    try:
...
def load_plan(workdir) -> dict:
    ...
    """
    merged: dict = {}
    for name, schema_name, key in FILES:
        doc = read_validated(Path(workdir) / name, schema_name)
        if key is None:
