#!/usr/bin/env python3
"""Materialize scaffold-specification.json agents[] signal lists from plan-data.json.
...
Group). This script fills each agent's interface.signals + transaction.fields
...
they must never be rand txn fields. The script also derives primary_clock (from the
§1.6 relationship=="primary" clock) and reset (from the §1.4.1 role=="reset" interface)
and writes them into the scaffold. It also materializes each testpoints[].inlined_check_hints[]
deterministically from the testpoint's covers[] + plan-data check_hints[], using
implementation_detail = verbatim-if-present-else-summary, so the LLM never hand-copies
the formula. Fails loud (SystemExit) when an agent omits interface_groups, names an unknown
group, has a duplicate group, a width is non-numeric, an interface row has an empty Role,
...
def _parse_width(raw, signal_name: str) -> int:
    try:
        ...
        sys.exit(
            f"materialize_scaffold: signal {signal_name!r} has non-numeric width {raw!r} "
...
def _clk_rst_signal_names(plan_data: dict) -> set:
    ...
    for s in plan_data.get("interfaces", []):
        ...
        if not role:
            sys.exit(
                f"materialize_scaffold: interface signal {s.get('signal_name')!r} has empty Role "
...
def _derive_primary_clock(plan_data: dict) -> dict:
    ...
    if not clocks:
        sys.exit(
            "materialize_scaffold: plan-data.json has no clocks[] — primary_clock cannot be "
    ...
    if primary is None:
        sys.exit(
            "materialize_scaffold: no clock with Relationship=='primary' in design.md §1.6 — "
    ...
    if not name or not period:
        sys.exit(
            f"materialize_scaffold: primary clock row missing clock_name/period_ns "
...
def _derive_reset(plan_data: dict) -> dict:
    ...
    sys.exit(
        "materialize_scaffold: no interface with Role=='reset' in design.md §1.4.1 — "
...
def _materialize_inline(plan_data: dict, scaffold: dict) -> None:
    ...
    for tp in scaffold.get("testpoints", []):
        ...
        for cid in tp.get("covers") or []:
            ...
            if h is None:
                sys.exit(
                    f"materialize_scaffold: testpoint {tp.get('id')!r} covers unknown check_id "
...
def materialize(plan_data: dict, scaffold: dict) -> dict:
    ...
    for agent in scaffold.get("agents", []):
        ...
        if not groups:
            sys.exit(
                f"materialize_scaffold: agent {aname!r} has no interface_groups. Each agent must "
        ...
        if unknown:
            sys.exit(
                f"materialize_scaffold: agent {aname!r} references unknown interface_group(s) "
                f"{unknown}. Valid groups in plan-data.json: {valid_groups}."
            )
        if len(groups) != len(set(groups)):
            dupes = sorted({g for g in groups if groups.count(g) > 1})
            sys.exit(
                f"materialize_scaffold: agent {aname!r} has duplicate interface_groups "
                f"entries: {dupes}."
            )
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Materialize scaffold-specification.json agents[] signals from plan-data.json."
    )
    p.add_argument("--plan-data", required=True, help="Path to plan-data.json")
    p.add_argument(
        "--scaffold",
        required=True,
        help="Path to scaffold-specification.json (written in place)",
    )
    args = p.parse_args()

    plan_data = json.loads(Path(args.plan_data).read_text(encoding="utf-8"))
    ...
    print(
        f"materialize_scaffold: materialized {len(scaffold.get('agents', []))} agent(s) "
        f"from {args.plan_data}"
    )
    return 0
