...
bash ${CLAUDE_PLUGIN_ROOT}/skills/simulation/scripts/bootstrap_simulation.sh \
    ...
    --plan asic/<M>/Verification/simulation-plan/scaffold-specification.json
```
...
4. When `--plan` is provided, invoke `derive_scaffold.py` to generate the full UVM scaffold from `scaffold-specification.json` (agent / driver / monitor / refmodel / scoreboard / env / tb_top / tb_pkg / filelist / generated_tests / tests/testlist.json).
