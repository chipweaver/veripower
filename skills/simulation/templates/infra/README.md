...
1. Copy this directory to `{workdir}` NO-CLOBBER (a carried file already present, brought forward by `carry_self`, always wins over the template); sed-substitute `MY_TOP` / `MY_MODULE` / `MY_RTL_DIR` (the absolute injected rtl-design root).
2. Generate `rtl_filelist.f` from the injected rtl-design `filelist.txt` (paths prefixed by the absolute `MY_RTL_DIR` root) — always regenerated, never no-clobbered.
...
| `env.sh` | `MODULE` / `TOP` / `RTL_DIR` / `UVM_HOME` / VCS-related environment variables. |
...
| `MY_RTL_DIR` | The absolute rtl-design stage root, from the injected `inputs.json` `rtl` key. |
