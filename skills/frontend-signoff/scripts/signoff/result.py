#!/usr/bin/env python3
"""result.py — frontend-signoff gate, envelope, and machine docs (the finalize verb).
...
def build_result(workdir: str, module: str) -> int:
    ...
    to finalize() -> exit 2 (the BLOCKED path): the script could not operate.
    """
    ...
    tmp.replace(workdir_p / "result.json")  # atomic: never observed half-written
    sys.stdout.write(
...
def finalize(workdir, module) -> int:
    """Public entry. Exit 0 = result.json written (pass or fail); exit 2 = BLOCKED
    (any internal raise) — never conflated with a status=fail verdict."""
    try:
        return build_result(workdir, module)
    except Exception as exc:  # noqa: BLE001 — any failure to operate is BLOCKED
        print(f"[signoff finalize] FAIL=internal {exc}", file=sys.stderr)
        return 2
