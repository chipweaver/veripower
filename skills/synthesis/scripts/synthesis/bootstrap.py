...
def _write_config_tcl(dest: Path, top: str) -> None:
    ...
    then dies on `FILL_IN_LIB_DB_PATH` — a file the caller never named. WIRE_LOAD_MODEL is
    written the same way and for the same reason."""
    lib_db = os.environ.get("LIB_DB") or "FILL_IN_LIB_DB_PATH"
    wlm = os.environ.get("WIRE_LOAD_MODEL") or "FILL_IN_WIRE_LOAD_MODEL"
    (dest / "scripts" / "config.tcl").write_text(
        ...
        "}\n"
        'if {![info exists ::env(WIRE_LOAD_MODEL)] || $::env(WIRE_LOAD_MODEL) eq ""} {\n'
        f'    set ::env(WIRE_LOAD_MODEL) "{wlm}"\n'
        "}\n"
    )
...
_LOCAL_SDC_STUB = """# ==============================================================================
# constraints.local.sdc — this stage's own SDC, sourced after the specification
# seed. The only SDC file you edit, and the only one carried into the next round.
#
# Tcl takes the last assignment, so a value set here settles a placeholder the
# seed left without either file having to know about the other.
#
# What belongs here:
#   - the timing exceptions and generated clocks constraint-annotations.json
#     declares, resolved against the real RTL (start/end points, divide ratios)
#   - set_clock_uncertainty -setup / -hold, once measured
#   - set_drive / set_load, where the IO cell library documents them
# What does NOT: clocks and IO delays (the seed's — a correction goes to
# specification and arrives here on its own).
#
# Anything you leave unset, and anything you decide not to add, needs a
# `# notes:` line saying why: this file is promoted, and the next reader cannot
# tell a measured margin from a default or an omission from an oversight.
# ==============================================================================
"""


def _assemble_sdc(dest: Path, seed: Path) -> None:
    """Write constraints.sdc = the specification seed + constraints.local.sdc.

    Rebuilt every round and never carried, so a specification-side correction to a clock or
    an IO delay reaches dc_shell on the round the kernel schedules for it. Before the split
    the two lived in one carried file, and the seed was read exactly once — on the first
    round — after which nothing re-read it and a corrected clock silently never arrived.

    Only constraints.local.sdc is this stage's, and only it is carried. Tcl takes the last
    assignment, so a `set_clock_uncertainty` there settles a placeholder the seed left
    without either file having to know about the other.
    """
    # Not shipped in templates/: the deploy is a copytree that overwrites, which would
    # discard a carried one. Written here only when absent.
    local = dest / "constraints.local.sdc"
    if not local.is_file():
        local.write_text(_LOCAL_SDC_STUB)
    (dest / "constraints.sdc").write_text(
        "\n".join(
            [
                "# ===========================================================================",
                "# constraints.sdc — GENERATED by the synthesis bootstrap verb, every round.",
                "# Do not edit: the next round overwrites it. Yours is constraints.local.sdc.",
                "# ===========================================================================",
                "",
                f"# --- specification: constraints/{seed.name} (verbatim) ---",
                seed.read_text(encoding="utf-8").rstrip("\n"),
                "",
                "# --- this stage: constraints.local.sdc ---",
    ...
    print(
        f"[synthesis bootstrap] constraints.sdc: assembled from constraints/{seed.name}"
