...
- When `--top` is omitted, it is read from `manifest.module`.
- When `<rtl>/rtl-files.json` is missing or names no RTL files, the script fails closed (avoiding a fallback to `${TOP}.v` that would mis-name a `.sv` top).
