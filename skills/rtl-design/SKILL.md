...
- **Freeze-reuse when nothing changed.** When the incremental-update branch's `specification` diff comes back empty and no `{directive_path}` is injected, do not regenerate anything — reuse the prior run's RTL / `filelist.txt` / `README.md` **and the prior `semantic-review.json`** verbatim (copy them forward; never ask a child sub-Task to reproduce them byte-for-byte, which a probabilistic model cannot guarantee). **On this freeze branch, SKIP the Step-4.4 semantic gate wave entirely and copy the prior `semantic-review.json` forward** — do not re-run the per-child intent review. Re-judging byte-identical RTL regenerates `semantic-review.json`, whose fingerprint is the pin's content selector (§5.4), so a fresh review would silently drop any `pin` on it (§4.3 "重验不重生成"). This is the generalization of `simulation`'s freeze branch to RTL AND its LLM judgment record — a forced full rebuild re-verifies existing conclusions, it does not regenerate them.
