...
| `semantic-review.json` | `references/semantic-review.schema.json` | Advisory per-child intent-review findings (every clean-gate finalize; non-gating) — aggregated by the main thread from the review wave. |
...
### Step 4: Finalize (scripts + gates + advisory review) + result.json
...
**4.4 Semantic review wave (advisory)** — runs on EVERY finalize that reaches a clean gate (NOT
first-run-only; closes the gap where a module that failed C on attempt 1 — promoted-on-fail, then retried
incrementally — would otherwise never be semantically reviewed):
...
- `STATUS: BLOCKED` OR malformed/unparseable JSON → record a `{child, severity:"minor",
  category:"unavailable", location:"-", summary:"review unavailable: <reason>"}` finding — never silently
  treated as ok, but a DISTINCT category from substantive concerns.
- `verdict="concerns"` iff any finding **with category ≠ `unavailable`**; `has_critical` iff any
  `severity=critical`.

Run `${CLAUDE_SKILL_DIR}/scripts/validate_semantic_review.py {workdir}/semantic-review.json` before adding it to `artifacts[]` (fix-and-retry on a non-zero exit). **This wave NEVER changes
`status`, and any failure inside it (dispatch / reap / aggregate / self-validate error) is caught and MUST
NOT block result.json assembly** — on a B-side error, omit `semantic-review.json` from `artifacts[]`, note
it in the completion summary, and proceed (B is strictly advisory).

**4.5 Assemble `result.json`** (`{workdir}/result.json`; schema `references/result.schema.json` + envelope):
`status`/`artifacts` from the gates (4.2/4.3 verdict + `semantic-review.json` when 4.4 produced one). In the
completion summary, emit one line `semantic-review: <ok | N children flagged | unavailable>; see
semantic-review.json`; **if `has_critical`**, add `⚠ <child> likely wrong-behavior — recommend operator
review before downstream` (advisory; operator decides — not a gate).

rtl-design failures route by **fix-locus**. **(1) Upstream / architecture** (`validate_rtl_exit` topology,
`<child>.md §2` incomplete, PPA, `build_*` unexpected error) → `status=fail` + a `fail_reason` naming the
upstream locus; **no internal loop, operator-driven** (the main thread stays a pure dispatcher and does not
self-loop on an upstream defect). **(2) Child-authoring defect** (`check_rtl_conformance` spec↔RTL presence
violations, or a mid-loop child `BLOCKED`) → fix-locus is the child itself, so it runs the **bounded
body-blind self-converge loop** (Step 4.3: hold the verdict, re-dispatch the failing children, re-run the
scripts, ≤2 rounds); exhausting the bound (or a mid-loop BLOCKED) falls back to (1)'s `status=fail`. The
loop is intra-stage fan-out (skill-internal; re-dispatches are not externally visible); finalize keeps no persistent "pending
finalize" state — bound exhaustion is terminal fail.
...
- [ ] **Semantic review (every clean-gate finalize):** the review wave ran, `semantic-review.json` was written + schema-self-validated + listed in `artifacts[]`; BLOCKED/malformed reviewers recorded as "review unavailable" (not silently ok); any `has_critical` surfaced; a B-side error did not block result.json. (Advisory — never gates `status`.)
...
- [`references/rtl-review-task-contract.md`](references/rtl-review-task-contract.md) — per-child semantic review sub-Task contract (advisory; dispatched in Step 4.4).
