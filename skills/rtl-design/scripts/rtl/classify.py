#!/usr/bin/env python3
"""rtl classify-delta — select the freeze branch: first-run | freeze | proceed.
...
RTL AND that run passed — then `rtl seed` copies the prior RTL/filelist/README AND
semantic-review.json forward, and the Step-4.4 semantic gate is skipped, keeping any pin alive.

input_digest() is the single home for the digest algorithm; rtl.result imports it so
...
def classify_delta(canonical_result, spec_dir) -> dict:
    ...
    if cr is None or not cr.is_file():
        return {"verdict": "first-run", "reason": "no canonical rtl-design result"}
    try:
...
def run(canonical_result, spec_dir) -> int:
    if not Path(spec_dir).is_dir():
        print(f"[rtl classify-delta] missing spec dir: {spec_dir}", file=sys.stderr)
        return 1
