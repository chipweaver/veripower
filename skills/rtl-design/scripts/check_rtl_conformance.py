#!/usr/bin/env python3
"""rtl-design conformance gate: spec↔RTL presence-level checks (Tier 2).
...
Usage:
  check_rtl_conformance.py --workdir W --manifest M --top T --ledger L --design D
...
def check_top_integration(workdir, manifest, top, ledger, design_text) -> list:
    ...
    if topc is None:
        return v  # coverage failure already caught by validate_rtl_exit
...
def check_module_presence(workdir, manifest, ledger) -> list:
    ...
    for child in manifest["children"]:
        ...
        rec = ledger.get(name)
        if rec is None:
            continue  # blocked/absent child is validate_rtl_exit's job, not here
...
def main() -> int:
    ...
    ap.add_argument("--top", required=True)  # used from Task 3 (top-integration check)
    ap.add_argument("--ledger", required=True, type=Path)
    ap.add_argument("--design", required=True, type=Path)  # used from Task 3
    a = ap.parse_args()

    try:
        manifest = _read_json(a.manifest)
        ledger = load_ledger(a.ledger)
        design_text = a.design.read_text(encoding="utf-8")
    ...
    violations = (
        check_module_presence(a.workdir, manifest, ledger)
        + check_annotation_reality(a.workdir, ledger)
        + check_top_integration(a.workdir, manifest, a.top, ledger, design_text)
    )
