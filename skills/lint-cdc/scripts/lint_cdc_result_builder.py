#!/usr/bin/env python3
"""lint_cdc_result_builder.py — assemble the lean lint-cdc result.json envelope.
...
def _write(workdir: Path, env: dict) -> None:
    ...
    sys.stdout.write(
        f"[lint_cdc_result_builder] Written: {workdir / 'result.json'}"
        f" (status={env['status']})\n"
    )
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(
        prog="lint_cdc_result_builder.py",
        description="Assemble the lean lint-cdc result.json from the two "
        "*-violations.json + report headers.",
    )
    ap.add_argument("--workdir", required=True, type=Path)
    ap.add_argument("--module", required=True)
    ap.add_argument("--top", default=None, help="top module; defaults to env.sh/$TOP")
    try:
        a = ap.parse_args(argv[1:])
    except SystemExit as exc:
        if exc.code not in (0, None):
            print("[lint_cdc_result_builder] ERROR: usage", file=sys.stderr)
            return 2
        return 0
    try:
        return run(a.workdir, a.module, top=a.top)
    except Exception as exc:  # noqa: BLE001 — any failure to operate is BLOCKED, not status=fail
        print(f"[lint_cdc_result_builder] FAIL=internal {exc}", file=sys.stderr)
        return 2
