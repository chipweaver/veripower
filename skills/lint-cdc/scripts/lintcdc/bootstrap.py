...
def _assemble_sgdc(dest: Path, seed: Path, rtl_dir: Path) -> int:
    """Write scripts/constraints.sgdc = seed + generated annotations + scripts/local.sgdc.

    Rebuilt every round and never carried, so a specification-side correction to the
    clock/reset block reaches the tool on the round the kernel schedules for it. Only
    local.sgdc is this stage's; splitting it out is what lets the derived part be replaced
    without touching a round's own work, which is why the derived part can be replaced at
    all.
    """
    annotations = _annotation_lines(rtl_dir)
    if annotations is None:
        return 1
    local = dest / "scripts" / "local.sgdc"
    body = [
        "# =============================================================================",
    ...
    print(
        ...
        " + constraint-annotations.json + scripts/local.sgdc"
    )
    return 0
