...
- **Inter-module behavior contract** (a shared operating-phase / sequencing / co-assertion contract, declared in the §1.3.2.1 companion): reference the companion's declared names; do not redefine the phase set or sequencing.
...
The hints live in `check-hints.json`, authored beside you from the ledger and the boundary;
the contract is `check-hints-task-contract.md` and the shape `check-hints.schema.json`. Keep this
section as a pointer to it; narrative about *why* a check exists belongs in §3 / §4.
