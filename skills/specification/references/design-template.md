...
| `check-hints/<child>.json` fields `check_id` / `source_feature` / `implementation_detail` / `observable` / `reference_rule` present and non-empty — **(schema)** | `check-hints/*.json` | Enforced by `check-hints.schema.json` at `check-coverage`; without them no rule-based RM / scoreboard can be generated. `implementation_detail_verbatim` carries no shape requirement; its fidelity is judged by the spec-review faithfulness lens against the whole brainstorm. |
| Every child's `brainstorm_anchor` resolves against `brainstorm.md` — **(gated)** | `manifest.json` + `brainstorm.md` | It is the spec-review reviewer's read-scope: an unparseable, out-of-bounds or inverted anchor makes a gating reviewer judge a child against blank or wrong text and report no findings. Enforced by `check-coverage`. |
