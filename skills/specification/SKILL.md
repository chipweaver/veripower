...
- **Freeze-reuse when nothing changed.** When you are re-dispatched with no `{directive_path}` and `brainstorm.md`'s content is unchanged from the run that produced the canonical `design.md` on disk, do not regenerate anything — reuse the prior run's `design.md` / `<child>.md` set / `manifest.json` / constraint files **and the prior `spec-review.json`** verbatim (copy them forward; never ask the LLM to reproduce them byte-for-byte, which a probabilistic model cannot guarantee). **Reuse the review record too, do not re-run the Step-7 semantic gate:** re-judging identical content regenerates `spec-review.json`, whose fingerprint is the pin's content selector (§5.4), so a fresh review would silently drop any `pin` on it (§4.3 "重验不重生成"). This is the generalization of `simulation`'s freeze branch to specification's authored artifacts AND its LLM judgment record — a forced full rebuild re-verifies existing conclusions, it does not regenerate them.
...
- **Trigger-driven rework** (`{rework_trigger}` injected): read its
  `stage_specific` once (field names per the triggering stage's own result schema); dispatch a design.md-level rework sub-Task that
