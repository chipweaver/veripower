...
Dispatch one sub-Task per child, each writing `children/<child>.md` per `references/child-design-template.md`, and alongside them one sub-Task writing `check-hints.json` per `references/check-hints-task-contract.md`. Inject each child's wire list from `derive-ports`; the child adds any top-IO ports it drives or reads. The hints are authored here rather than earlier because a row the human just ruled on can have become one simulation judges.
