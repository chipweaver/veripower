...
Rules:
- **Every scheduling decision lives in `schedule.py`, never in the Orchestrator or a skill.** "If upstream is X, start Y" is a decision — it is computed by `kernel.py decide`; the Orchestrator only executes the one action it returns. Deterministic rework-target *selection* is the one routing computation extracted to a pure sibling script, `route.py`, composed unchanged inside `schedule.py` and `kernel.py`. The Orchestrator's only judgment channel is the ask-gated `pin` / `reopen` / human `diagnose` proposals — nothing else. It authors no per-dispatch content: at dispatch time every fact it could state is already a file the target reads, so it passes coordinates and the kernel resolves them into `dispatch.json`.
...
- Unit tests live under `tests/unit/`: `test_kernel_cli.py` (verbs), `test_schedule.py` (decide), `test_rules.py` (registry + derived graph), `test_facts_*.py` (events / fingerprints / freshness), `test_route.py` (routing maps). Cover happy path + error branches.
...
**File naming** — each stage's Tier-1 scripts live in one package `skills/<stage>/scripts/<pkg>/` (`<pkg>` = the short stage tool name, no hyphens, e.g. `lint-cdc`→`lintcdc`): a thin `__main__.py` argparse entry dispatches verb subcommands to focused `<verb>.py` / `_<lib>.py` modules (no god-file). Verbs follow the shared vocabulary (`bootstrap` / `finalize` / `derive-*` / `materialize-*` / `render-*` / `check-*` / `validate-review`). Framework libraries keep domain-noun names (`rules.py`, `store.py`); kebab-case skill directories.
