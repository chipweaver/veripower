...
**File naming** — each stage's Tier-1 scripts live in one package `skills/<stage>/scripts/<pkg>/` (`<pkg>` = the short stage tool name, no hyphens, e.g. `lint-cdc`→`lintcdc`): a thin `__main__.py` argparse entry dispatches verb subcommands to focused `<verb>.py` / `_<lib>.py` modules (no god-file). Verbs follow the shared vocabulary (`bootstrap` / `finalize` / `derive-*` / `materialize-*` / `render-*` / `check-*` / `validate-review`). `validate-review` is not a default: specification and simulation-plan both dropped it when their reviewers began writing their own files, because the verb was validating a record the main thread had just re-typed, and the verdict it computed was then checked against that same thread's own `--status`. Add it only where a review record has a reader other than the party being checked. Framework libraries keep domain-noun names (`rules.py`, `store.py`); kebab-case skill directories.
